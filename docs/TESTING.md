...
| `ui.CanvasWidgetTest` | `gui;gpu` | Qt Test | Real widget + OpenGL + session: scripted mouse/wheel drawing aligned with world coordinates, ink visible in the framebuffer, undo action, reopen with identical ink; blank/ruled/grid/dot backgrounds and bounded pages by pixel sampling, selection frame, debug HUD on/off; ink stays under the pointer after resizes and the framebuffer is logical size × DPR; the eraser ring platform cursor (centred hot spot, applied on tool change; runs without OpenGL too). The OpenGL cases skip without a 3.3 context (the offscreen platform in CTest/CI); run `canvas_widget_tests` directly on a desktop (`STUDYAPP_TEST_SCREENSHOTS=<dir>`, `STUDYAPP_TEST_THEME=dark`) |
